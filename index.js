...
const {
  ...
  RG_RUC_PANCAKE_SWAP, RG_USDT_PANCAKE_SWAP
} = require('./config');
...
RG_RUC_PANCAKE.on('Swap', async (sender, amount0In, amount1In, amount0Out, amount1Out, to, event) => {
  ...
  // RUC 卖出数量
  const amount0InFmt = formatAmount(amount0In);
  // 获得RG数量
  const amount1OutFmt = formatAmount(amount1Out);

  // 卖出RG数量
  const amount1InFmt = formatAmount(amount1In);
  // 获得RUC数量
  const amount0OutFmt = formatAmount(amount0Out);
